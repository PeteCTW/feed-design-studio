...
const NewsCard = ({ image, category, title, excerpt, author, readTime, index, tags, citations, verifications }: NewsCardProps) => {
  ...
  return (
    <motion.article
      ...
      className="group cursor-pointer bg-card rounded-lg border border-border hover:border-accent/30 transition-all hover:shadow-lg"
    >
      ...
      <div className="p-5">
        ...
        <h3 className="font-display text-lg font-bold mt-2 leading-snug group-hover:text-accent transition-colors">
          {title}
        </h3>
        ...
        <div className="mt-3 flex flex-wrap gap-1">
          {citations.slice(0, 3).map((cite, i) => (
            <a
              ...
              className="inline-flex items-center gap-1 font-body text-[10px] text-muted-foreground hover:text-accent transition-colors bg-secondary/60 px-2 py-0.5 rounded-full"
            >
